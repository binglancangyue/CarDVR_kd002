...
public class Mp4ParseUtil {
     ...
     */
    public static void appendMp4List(List<String> mp4PathList, String outPutPath, ObservableEmitter<String> emitter) {
        ...
        try {
            ...
            Log.d("Mp4ParseUtil", "appendMp4List: " + outPutPath);
            Container outContainer = new DefaultMp4Builder().build(resultMovie);// 将结果Movie对象封装进容器
            FileChannel fileChannel =
                    new RandomAccessFile(String.format(outPutPath), "rw").getChannel();
...
//            filePath = outPutPath;
            ...
            File first = new File(mp4PathList.get(0));
            first.delete();
            first = new File(mp4PathList.get(1));
            first.delete();
            Log.d("Mp4ParseUtil", "appendMp4List:delete ");
        }
//        return filePath;
    }
    ...
    public static void cropMp4(String mp4Path, long fromSample, long toSample, String outPath) {
        ...
        try {
            ...
            Movie resultMovie = new Movie();
            resultMovie.addTrack(new AppendTrack(new CroppedTrack(videoTracks, fromSample,
                    toSample)));// 视频部分
            resultMovie.addTrack(new AppendTrack(new CroppedTrack(audioTracks, fromSample,
                    toSample)));// 音频部分
